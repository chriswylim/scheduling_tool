...
class UserMan extends React.Component {
  ...
  render() {
    ...
    const activeProtegesList = activeProteges.map(ninja => {
      return (
        <ProtegeCard ninja={ninja} onSubmit={this.props.EditAProtege} onSuspend={this.props.SuspendAProtege} />
      )
    });

    const suspendedProtegesList = suspendedProteges.map(samurai => {
      return (
        <ProtegeCard ninja={samurai} onRevive={this.props.SuspendAProtege} onDelete={this.props.PermaDeleteAProtege} />
      )
    })
